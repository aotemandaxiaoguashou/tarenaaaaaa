...
<html>
	...
	<link href="/rd/Public/css/layout.css" rel="stylesheet" type="text/css">
	
	<style>
.user_order_search input {
    float: left;
...
.orange_buttom_4 {
    background: url("themes/kujie/images/button_bg_1.gif") no-repeat scroll -54px 0 #f79800;
    width: 67px;
}
.orange_buttom_2, .orange_buttom_4, .orange_buttom_6, .orange_buttom_8, .orange_buttom_10 {
    border: medium none;
    color: #fff;
    cursor: pointer;
    height: 21px;
...
.white_buttom_2 {
    background: url("themes/kujie/images/button_bg_2.gif") no-repeat scroll 0 0 #f79800;
    width: 51px;
}
</style>
...
<!--*********(BLOCK=eachCONTENT)开始，各页面中间的自己独有的部分*****************************************************************************************-->
  ...
  <span>当前位置：</span><a href="/rd/index.php/Index/index">首页</a>&gt; <em>个人中心</em>
</div>
<div class="w clearfix mb15">
	...
	<div class="maincont fr">
		<div class="user-title">订单中心</div>
		<div class="main_cont">
			<div class="user_default p15">
				<div class="user_tip_order">
				  <p>订单号：<?php echo ($ord["ident"]); ?></p>
				  <p>下单时间：<?php echo (date("Y-m-d H:i",$ord["addtime"])); ?></p>
				  <p>订单状态：<em><?php if($ord["issure"] == 0): ?>未审核<?php else: ?>已审核<?php endif; ?></em>
						  <em><?php if($ord["ispay"] == 0): ?>未支付<?php else: ?>已支付<?php endif; ?></em>
						  <em><?php if($ord["issend"] == 0): ?>未发货<?php elseif($ord["issend"] == 1 and $ord["isgets"] == 1): ?>已收货<?php elseif($ord["issend"] == 1 and $ord["isgets"] == 0): ?>已发货<?php endif; ?></em></p>
				  <div class="clearfix go-pay">
					<a href="/rd/index.php/Home/User/index"><input value="返回" class="return gopay-btn white_buttom_2"  type="button"></a>
					<a href="/rd/index.php/Indent/goPay"><input value="立即付款" class="fukuan gopay-btn orange_buttom_4" type="button"></a>
				  </div>
				</div>
				<div class="tip_collection">
					<table class="user-tab6" width="100%" cellpadding="0" cellspacing="0">
					  <tbody>
						<tr>
						  <th width="10%">商品图片</th>
						  <th width="40%">商品名称</th>
					 ...
					 <?php if(is_array($gids)): foreach($gids as $key=>$g): ?><tr>
						...
						<tr>
						  <td colspan="8" class="td_s" align="right" bgcolor="#f6f6f6">
							<p>商品总价：￥<?php echo ($ord["all-price"]); ?></p>
							<p><b>实际支付金额：￥<?php echo ($ord["all-price"]); ?><font color="#ff1b71"></font></b></p>
							<p>您本次订单获得的积分：<?php echo ($ord["points"]); ?></p>                    </td>
						</tr>
					</tbody>
				  </table>
				</div>
          <div class="clearfix p15">
            <div class="myintegral pb5">收货人信息</div>
            <table class="user-tab7" width="100%" border="0" cellpadding="0" cellspacing="4">
              <tbody>
                  <tr>
                      <td width="8%" align="right">姓名：</td>
                      <td width="92%"><?php echo ($users["tname"]); ?></td>
                  </tr>
               
                  <tr>
                    <td width="8%" align="right">地址：</td>
                    <td width="92%"><?php echo ($users["address"]); ?></td>
                  </tr>
                  <tr>
                    <td width="8%" align="right">邮编：</td>
                    <td width="92%"><?php echo ($users["zipcode"]); ?></td>
                  </tr>
                  <tr>
                    <td width="8%" align="right">手机：</td>
                    <td width="92%"><?php echo ($users["phone"]); ?></td>
                  </tr>
