...
<title>会员中心-首页-酷结网</title>
	...
	<script src="/rd/Public/js/scrollpic.js" type="text/javascript"></script>
...
<div class="w clearfix mb15">
  ...
  <div class="maincont fr">
    ...
    您好 <em><?php echo ($_SESSION[C('SESSION_USER')]['username']); ?></em> 欢迎您回来！
    </div>
    <div class="user-content">
      <dl class="user-level clearfix p10">
        <dt>您目前的级别：</dt>
        <dd><p><em>初始会员</em></p>
      </dl>
	
      <div class="clearfix p10">
        <h2 class="order-hd mb10">订单列表</h2>
        <table class="user-tab1 usertab" width="100%" border="0" cellpadding="0" cellspacing="0">
          <thead>
            <tr>
              <th width="20%">订单号</th>
          ...
          <tbody>
		  <?php if(is_array($orders)): $i = 0; $__LIST__ = $orders;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ord): $mod = ($i % 2 );++$i;?><tr>
              <td><a href="/rd/index.php/Home/User/ordDetail/id/<?php echo ($ord["id"]); ?>"><?php echo ($ord["ident"]); ?></a></td>
              <td><?php echo ($ord["all-price"]); ?></td>
              <td><?php echo (date("Y-m-d H:i",$ord["addtime"])); ?></td>
              <td><?php if($ord["issure"] == 0): ?>未审核<?php else: ?>已审核<?php endif; ?></td>
              <td><?php if($ord["ispay"] == 0): ?>未支付<?php else: ?>已支付<?php endif; ?></td>
              <td><?php if($ord["issend"] == 0): ?>未发货<?php elseif($ord["issend"] == 1 and $ord["isgets"] == 1): ?>已收货<?php elseif($ord["issend"] == 1 and $ord["isgets"] == 0): ?>已发货<?php endif; ?></td>
              <td><?php if($ord["ispay"] == 0): ?><a href="/rd/index.php/User/dopay/id/<?php echo ($ord["id"]); ?>" target="_blank">付款</a><?php elseif($ord["ispay"] == 1 and $ord["issend"] == 0): ?>等待发货
					<?php elseif($ord["ispay"] == 1 and $ord["issend"] == 1 and $ord["isgets"] == 0): ?><a href="/rd/index.php/Indent/dogets/id/<?php echo ($ord["id"]); ?>">确认收货</a><?php endif; ?></td>
